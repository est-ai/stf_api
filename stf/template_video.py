...
def preprocess_template2(config_path,
    ...
    if not Path(crop_mp4).exists():
        ...
        g_anchor_ebd = df_face['ebd'].values[0]
        # 템플릿 동영상에서 아나운서 얼굴 위치만 저장해 놓는다
        df_paths = ff.save_face_info3(template_video_path, g_anchor_ebd, base=preprocess_dir, verbose=verbose)

        ### 얼굴 영역을 FAN 랜드마크 기반으로 크롭해 놓는다
        assert len(df_paths) == 1
        if verbose:
            print('cwf.save_crop_info2 --')
        df_fan_path = cwf.save_crop_info2(anchor_box_path=df_paths[0], mp4_path=template_video_path, out_dir=crop_mp4, crop_offset_y = config.crop_offset_y, crop_margin = config.crop_margin, verbose=verbose, )
        # snow : for debug
        if verbose:
            print('df_fan_path: ', df_fan_path)
    else:
